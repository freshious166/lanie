  ...
  return (
    <div className="flex flex-col pb-32 bg-background-dark min-h-screen">
        ...
        <div className="mt-6 sticky top-16 bg-background-dark z-40">
          <div className="flex border-b border-[#324867] px-4 justify-between">
            {['Services', 'Reviews', 'About'].map((tab, i) => (
              <button key={tab} className={`flex flex-col items-center justify-center border-b-[3px] py-4 flex-1 ${i === 0 ? 'border-primary text-white' : 'border-transparent text-gray-400'}`}>
        ...
        <div className="mt-2">
          ...
          <div className="space-y-1">
            {workshop.services.map(service => (
              <div key={service.id} className="flex items-center gap-4 bg-background-dark px-4 py-3 hover:bg-white/5 cursor-pointer border-b border-white/5 active:bg-white/10">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-background-dark/90 backdrop-blur-xl border-t border-white/10 flex items-center gap-3 pb-24 z-50">
        <button className="flex size-14 items-center justify-center rounded-xl bg-[#233348] text-white active:bg-[#2b3e57]">
          <span className="material-symbols-outlined">chat_bubble</span>
        </button>
        <button className="flex-1 h-14 bg-primary rounded-xl text-white font-bold text-lg shadow-lg shadow-primary/20 active:scale-95 transition-transform">
          Book Now
        </button>
      </div>
    </div>
