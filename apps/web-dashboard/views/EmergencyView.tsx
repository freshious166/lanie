...
const EmergencyView: React.FC = () => {
  ...
  return (
    <div className="relative flex h-screen flex-col bg-background-light overflow-hidden">
      {/* Top Header */}
      <div className="flex items-center p-4 pb-2 justify-between z-10">
        ...
        <h2 className="text-gray-900 text-lg font-bold flex-1 text-center pr-12">Emergency Assistance</h2>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pb-48">
        ...
        <div className="relative px-4 py-3">
          ...
          <div className="absolute bottom-6 left-8 right-8 bg-background-light/90 backdrop-blur-md border border-gray-200 rounded-lg p-3 flex items-center gap-3">
            <span className="material-symbols-outlined text-primary">my_location</span>
            <div className="flex flex-col">
              <span className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">Detected Location</span>
        ...
        {/* Headline */}
        <div className="pt-2">
          <h3 className="text-gray-900 text-2xl font-bold px-4 text-center">What's the emergency?</h3>
          <p className="text-gray-500 text-sm text-center px-8 mt-1">Select the type of assistance you need immediately.</p>
        </div>

        {/* Segmented Buttons */}
        <div className="flex px-4 py-6">
          <div className="flex h-12 flex-1 items-center justify-center rounded-xl bg-gray-100 p-1.5 shadow-inner">
            <button 
              onClick={() => setServiceType('Towing')}
              className={`flex h-full grow items-center justify-center rounded-lg px-2 transition-all font-bold text-sm ${serviceType === 'Towing' ? 'bg-primary text-gray-900 shadow-lg' : 'text-gray-500'}`}>
              Towing
            </button>
            <button 
              onClick={() => setServiceType('Roadside Repair')}
              className={`flex h-full grow items-center justify-center rounded-lg px-2 transition-all font-bold text-sm ${serviceType === 'Roadside Repair' ? 'bg-primary text-gray-900 shadow-lg' : 'text-gray-500'}`}>
              Roadside Repair
            </button>
          </div>
        </div>
          ...
          {issues.map(issue => (
            <div 
              ...
              className={`flex flex-col gap-3 rounded-xl border p-4 cursor-pointer transition-colors active:scale-95 ${
                selectedIssue === issue.id ? 'border-primary bg-gray-100' : 'border-gray-200 bg-white'
              }`}>
              <div className="text-primary">
                <span className="material-symbols-outlined text-3xl">{issue.icon}</span>
              </div>
              <div className="flex flex-col gap-0.5">
                <h2 className="text-gray-900 text-base font-bold leading-tight">{issue.label}</h2>
                <p className="text-gray-500 text-[12px] font-medium">{issue.sub}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="px-4">
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            <textarea 
              className="w-full bg-transparent border-none p-0 text-sm focus:ring-0 text-gray-900 placeholder:text-slate-500" 
      ...
      {/* Bottom CTA */}
      <div className="absolute bottom-0 left-0 right-0 p-4 pb-24 bg-background-light/80 backdrop-blur-xl border-t border-gray-100">
        <button className="w-full bg-primary hover:bg-primary/90 text-gray-900 font-bold py-5 rounded-2xl shadow-2xl shadow-primary/40 flex items-center justify-center gap-3 active:scale-[0.98] transition-all">
