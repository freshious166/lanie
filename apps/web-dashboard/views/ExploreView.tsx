...
const ExploreView: React.FC = () => {
  ...
  const { data: workshops } = useQuery({
    ...
    queryFn: async () => {
      const isVerified = activeFilter === 'Verified';
      const { data } = await axios.get(`${API_URL}/vendors?isVerified=${isVerified}`);
      return data.map((v: any) => ({
        ...
        address: v.address,
        distance: '1.2 mi', // In a real app we'd calculate this via geolocation
    ...
    <div className="relative flex flex-col h-screen overflow-hidden bg-background-dark">
      ...
      <div className="relative z-20 mt-auto flex flex-col bg-background-dark rounded-t-[32px] border-t border-white/10 shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex flex-col gap-4 p-5 max-h-[350px] overflow-y-auto no-scrollbar">
          {workshops && workshops.length > 0 ? workshops.map((workshop: any) => (
            <div 
              key={workshop.id} 
            ...
            <div className="text-center text-slate-500 py-4">No workshops found for this filter.</div>
          )}
