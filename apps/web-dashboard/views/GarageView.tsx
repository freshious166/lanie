...
const GarageView: React.FC = () => {
  ...
  const activeVehicle = useMemo(() => {
    return vehicles.find((v: any) => v.id === activeVehicleId) || vehicles[0];
  }, [vehicles, activeVehicleId]);
  ...
  if (isLoadingVehicles) {
    return <div className="flex h-screen items-center justify-center bg-background-light text-gray-900 font-bold">Loading Garage...</div>;
  }

  if (!activeVehicle) {
    return <div className="flex h-screen items-center justify-center bg-background-light text-gray-900 font-bold">No Vehicles Found</div>;
  }

  return (
    <div className="flex flex-col pb-24">
      {/* Top Header */}
      <header className="flex items-center p-4 pb-2 justify-between sticky top-0 z-50 bg-background-light/80 backdrop-blur-md">
        <div className="text-gray-900 flex size-12 items-center justify-start">
          <span className="material-symbols-outlined text-3xl">account_circle</span>
        </div>
        <h2 className="text-gray-900 text-lg font-bold flex-1 text-center">Personal Garage</h2>
      ...
      <div className="px-4 mt-2">
        <div className="flex items-center h-12 bg-surface-dark/50 rounded-xl border border-gray-200 px-4 gap-3">
          ...
          <input
            ...
            className="bg-transparent border-none focus:ring-0 text-gray-900 placeholder-[#324867] w-full text-sm font-medium p-0"
          />
          {searchQuery && (
            ...
            {filteredVehicles.map((vehicle) => (
              ...
              >
                <div className={`relative w-full aspect-video rounded-xl overflow-hidden shadow-lg border-2 transition-colors ${activeVehicleId === vehicle.id ? 'border-primary' : 'border-transparent'}`}>
                  ...
                  <div className="absolute top-3 left-3 bg-primary text-gray-900 text-[10px] font-bold px-2 py-1 rounded-full flex items-center gap-1">
                    <span className="material-symbols-outlined text-xs fill-1">verified</span> ACTIVE
                  </div>
                </div>
                <div>
                  <p className={`text-lg font-bold leading-tight transition-colors ${activeVehicleId === vehicle.id ? 'text-gray-900' : 'text-gray-500'}`}>{vehicle.name}</p>
                  <p className="text-gray-500 text-sm font-medium">{vehicle.model} • {vehicle.licensePlate}</p>
                </div>
              </div>
            ))}
          </div>
        ) : (
          <div className="px-4 py-8 text-center bg-surface-dark/30 mx-4 rounded-xl border border-gray-100 border-dashed">
            <span className="material-symbols-outlined text-[#324867] text-4xl mb-2">directions_car</span>
            <p className="text-gray-500 text-sm font-medium">No vehicles found matching your search.</p>
          </div>
        )}
            ...
            <button
              ...
              className={`flex h-9 shrink-0 items-center justify-center gap-2 rounded-full px-5 transition-all text-sm font-semibold border ${
                activeStatusFilter === filter
                  ? 'bg-primary border-primary text-gray-900 shadow-lg'
                  : 'bg-surface-dark border-gray-200 text-gray-500'
              }`}
            >
              {filter !== 'All' && (
      ...
      <div className="mt-6 px-4">
        <div className="bg-surface-dark rounded-2xl border border-gray-100 p-4 flex flex-col gap-4">
          <div className="flex items-center justify-between">
            <h3 className="text-gray-900 text-base font-bold flex items-center gap-2">
              <span className="material-symbols-outlined text-primary">qr_code_scanner</span>
              VIN Verification
            </h3>
            <span className="text-[10px] font-bold text-gray-500 tracking-widest uppercase">Safety First</span>
          </div>
          ...
          <div className="flex flex-col gap-2">
            <div className="relative">
              <input
                ...
                className={`w-full bg-[#101822] border rounded-xl py-3 px-4 text-gray-900 font-mono placeholder-[#324867] focus:ring-primary focus:border-primary uppercase transition-colors ${
                  error ? 'border-red-500/50 ring-1 ring-red-500/20' : 'border-gray-200'
                }`}
              />
              <div className="absolute right-4 top-1/2 -translate-y-1/2 text-[10px] text-gray-500 font-bold">
                {vinInput.length}/17
              </div>
            </div>
            {error && (
          ...
          <button
            ...
            className={`w-full h-12 rounded-xl flex items-center justify-center gap-2 font-bold transition-all ${
              isVerifying
                ...
                : 'bg-primary text-gray-900 shadow-lg shadow-primary/20 active:scale-95'
            }`}
          >
            {isVerifying ? (
            ...
            <div className={`mt-2 p-4 rounded-xl border animate-in fade-in slide-in-from-top-2 duration-300 ${
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex flex-col gap-1">
                  <p className="text-gray-900 text-sm font-bold">{verificationResult.message}</p>
                  {verificationResult.details && (
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <div className="flex flex-col">
                        <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Vehicle</span>
                        <span className="text-xs text-gray-900">{verificationResult.details.year} {verificationResult.details.make}</span>
                      </div>
                      <div className="flex flex-col">
                        <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Model</span>
                        <span className="text-xs text-gray-900">{verificationResult.details.model}</span>
                      </div>
                      <div className="flex flex-col col-span-2 mt-1">
                        <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Engine Specs</span>
                        <span className="text-xs text-gray-900">{verificationResult.details.engine}</span>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <div className="mt-6 px-4">
        <div className="bg-surface-dark rounded-2xl border border-gray-100 p-4 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="flex flex-col items-center justify-center p-3 bg-[#101822] rounded-xl border border-gray-100 gap-1">
              <span className="material-symbols-outlined text-gray-500 text-lg">speed</span>
              <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tight">Mileage</p>
              <p className="text-gray-900 text-sm font-bold">{activeVehicle.mileage.toLocaleString()}</p>
              <p className="text-[8px] text-gray-500 font-medium italic">mi total</p>
            </div>
            
            <div className="flex flex-col items-center justify-center p-3 bg-[#101822] rounded-xl border border-gray-100 gap-1 text-center">
              <span className="material-symbols-outlined text-gray-500 text-lg">event_note</span>
              <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tight">Next Svc</p>
              <p className="text-gray-900 text-sm font-bold">{activeVehicle.nextServiceMileage.toLocaleString()}</p>
              <p className="text-[8px] text-gray-500 font-medium italic">mi target</p>
            </div>

            <div className="flex flex-col items-center justify-center p-3 bg-[#101822] rounded-xl border border-gray-100 gap-1">
              <span className="material-symbols-outlined text-green-500 text-lg">payments</span>
              <p className="text-[10px] text-gray-500 uppercase font-bold tracking-tight">Value</p>
              <p className="text-gray-900 text-sm font-bold">{activeVehicle.value}</p>
              <p className="text-[8px] text-gray-500 font-medium italic">est. mkt</p>
            </div>
          </div>
          
          <div className="flex items-center gap-2 text-[10px] text-gray-500 bg-white/5 p-2 rounded-lg">
      ...
      {/* Vehicle Status */}
      <h3 className="text-gray-900 text-lg font-bold px-4 pb-2 pt-6">Vehicle Status</h3>
        ...
        <div className="flex items-stretch justify-between gap-4 rounded-xl bg-white dark:bg-surface-dark p-4 border border-slate-100 dark:border-transparent">
          <div className="flex flex-[2_2_0px] flex-col gap-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-gray-900 text-base font-bold leading-tight">Next Service in {Math.max(0, activeVehicle.nextServiceMileage - activeVehicle.mileage)} mi</p>
              <p className="text-gray-500 text-sm font-normal">Oil change and brake inspection</p>
            </div>
            <button className="flex min-w-[100px] items-center justify-center rounded-lg h-9 px-4 bg-primary/10 dark:bg-surface-accent text-primary dark:text-gray-900 gap-2 text-sm font-semibold">
        ...
        <div className="flex items-stretch justify-between gap-4 rounded-xl bg-white dark:bg-surface-dark p-4 border border-slate-100 dark:border-transparent">
          <div className="flex flex-[2_2_0px] flex-col gap-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-gray-900 text-base font-bold leading-tight">Insurance Expires in 30 days</p>
              <p className="text-gray-500 text-sm font-normal">Policy #LN-99283</p>
            </div>
            <button className="flex min-w-[100px] items-center justify-center rounded-lg h-9 px-4 bg-primary text-gray-900 gap-2 text-sm font-semibold">
      ...
      {/* Quick Actions */}
      <h3 className="text-gray-900 text-lg font-bold px-4 pb-2 pt-6">Quick Actions</h3>
